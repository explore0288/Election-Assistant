...
2. **Prompt Engineering & Persona:** The AI is strictly instructed via `systemInstruction` to adopt the persona of "Kartikey". It is restricted to answering *only* election-related queries to prevent misuse.
...
* **Voice Input (Speech-to-Text):** Using the browser's native `Web Speech API (SpeechRecognition)`, users can click the microphone icon to speak their questions. The app captures the audio, transcribes it using the `en` (Indian English/Hinglish) language model, and auto-sends the text to the AI.
* **Voice Output (Text-to-Speech):** Once the AI replies, the `window.speechSynthesis` API scans the user's operating system for high-quality male voices (prioritizing Indian English voices like Ravi, David, or Prabhat). It then reads the AI's response out loud with an adjusted pitch for a friendly, natural tone.
